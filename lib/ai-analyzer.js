...
const { ELEMENT_TYPE, CLUSTER_TYPE } = require('./utils/constants');
...
class AiAnalyzer
{
    ...
    )
    {
        ...
        let rawElements = detectedElements.map(el => ({absoluteTiles: el.tiles, type: ELEMENT_TYPE}));
    ...
    {
        let itemType = sc.get(raw, 'type', ELEMENT_TYPE);
        return {
            name: (CLUSTER_TYPE === itemType ? 'cluster-' : '')+Helpers.elementName(i + 1),
            type: itemType,
    ...
    async expandClusters(rawElements, detectedClusters, cropConfig, providers, onProgress)
    {
        ...
        for(let i = 0; i < total; i++){
            ...
            if(!providers.length){
                rawElements.push({absoluteTiles: cluster.tiles, type: CLUSTER_TYPE});
                continue;
            }
            ...
            if(!subElements.length){
                ...
                rawElements.push({absoluteTiles: cluster.tiles, type: ELEMENT_TYPE});
                continue;
            }
    ...
    addSubElements(rawElements, subElements, cluster)
    {
        for(let subElement of subElements){
            ...
            rawElements.push({absoluteTiles, type: ELEMENT_TYPE});
        }
    }
    ...
    async nameElements(rawElements, multiAi, cropConfig, providers, onToken, onProgress, validatePass)
    {
        ...
        for(let i = 0; i < rawElements.length; i++){
            ...
            elements.push({name: result.name, type: ELEMENT_TYPE, layers: absoluteLayers});
        }
        return elements;
    }

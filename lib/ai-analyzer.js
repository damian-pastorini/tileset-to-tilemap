...
class AiAnalyzer
{
    ...
    async analyzeImage(
        imageBuffer, tilesetColumns, tileRows, tileWidth, tileHeight, margin, spacing,
        providers, bgColorHex, onToken, onProgress, debugDir
    )
    {
        Logger.info(
        ...
        let rawElements = detectedElements.map(element => ({absoluteTiles: element.tiles, type: TilesetConst.ELEMENT_TYPE}));
        ...
        return {
            elements: multiAi.deduplicateNames(await this.nameElements(
                rawElements,
                multiAi,
                cropConfig,
                providers,
    ...
    {
        return {
            name: (TilesetConst.CLUSTER_TYPE === sc.get(raw, 'type', TilesetConst.ELEMENT_TYPE) ? 'cluster-' : '')
                +Helpers.elementName(i + 1),
            type: sc.get(raw, 'type', TilesetConst.ELEMENT_TYPE),
    ...
    toAbsoluteElement(element, cluster)
    {
        return {
            name: element.name,
    ...
    async cropClusterWithParams(imageBuffer, cluster, params)
    {
        return await this.cropper.crop(
            imageBuffer, cluster, params.tileWidth, params.tileHeight,
            params.margin, params.spacing, params.bgColor
        );
    }
    ...
    async assignLayersAbsolute(imageBuffer, elementTiles, params)
    {
        ...
        let cropResult = await this.cropClusterWithParams(imageBuffer, miniCluster, params);
        let layers = null;
        try {
    ...
    async detectClusterElements(imageBuffer, clusterTiles, params)
    {
        ...
        let cropResult = await this.cropClusterWithParams(imageBuffer, cluster, params);
        let detected = [];
        try {
        ...
        let elements = [];
        for(let element of detected){
            elements.push(this.toAbsoluteElement(element, cluster));
        }
        return elements;
    }
}

...
class TilesetFilesBuilder
{
    ...
    async buildSingleElementFiles(ctx, element)
    {
        ...
        FileHandler.writeFile(
            FileHandler.joinPaths(ctx.elementsDir, elementFilename), MapFormatter.formatMap(elementJson)
        );
        ...
        return [this.buildOutputEntry(ctx.sessionId, TilesetConst.OUTPUT_ELEMENTS_SUB_FOLDER+'/'+elementFilename)];
    }
    ...
    async buildElementFiles(sessionId, outputDir, tileset, imageBuffer, croppedDir, croppedElementsPaths)
    {
        ...
        FileHandler.createFolder(elementsDir);
        let ctx = {
            sessionId,
            outputDir,
            elementsDir,
    ...
    async loadTilesetImageBuffer(tileset, outputDir, genTimestamp)
    {
        ...
        let croppedDir = FileHandler.joinPaths(outputDir, 'cropped-elements', genTimestamp);
        FileHandler.createFolder(croppedDir);
        return {imageBuffer, croppedDir};
    }

    removeStaleCroppedElements(outputDir, keepTimestamp)
    {
        let croppedRoot = FileHandler.joinPaths(outputDir, 'cropped-elements');
        if(!FileHandler.exists(croppedRoot)){
            return;
        }
        for(let folder of FileHandler.fetchSubFoldersList(croppedRoot)){
            if(folder === keepTimestamp){
                continue;
            }
            FileHandler.remove(FileHandler.joinPaths(croppedRoot, folder));
        }
    }
    ...
    async buildTilesetFilesEntries(sessionId, outputDir, tilesets, genTimestamp, croppedElementsPaths, annotatedDir, generateAnnotated)
    {
        let annotatedImageBuilder = generateAnnotated ? new AnnotatedImageBuilder() : false;
        if(generateAnnotated){
        ...
        for(let tileset of tilesets){
            ...
            if(!sc.hasOwn(tileset, 'originalFilename')){
    ...
    buildCompositeEntries(sessionId, outputDir, tilesetsBySize, compositeOptions)
    {
        ...
        for(let sizeKey of sizeKeys){
            ...
            let configFullPath = FileHandler.joinPaths(outputDir, configFilename);
            let existingSavedWizardConfig = this.readSavedWizardConfig(configFullPath);
            if(existingSavedWizardConfig){
                configData.savedWizardConfig = existingSavedWizardConfig;
            }
            FileHandler.writeFile(configFullPath, sc.toJsonString(configData, null, 4));
            files.push(this.buildOutputEntry(sessionId, configFilename));
        }
        return files;
    }

    readSavedWizardConfig(configFullPath)
    {
        if(!FileHandler.exists(configFullPath)){
            return null;
        }
        let existingConfig = FileHandler.fetchFileJson(configFullPath);
        if(!existingConfig){
            return null;
        }
        return sc.get(existingConfig, 'savedWizardConfig', null);
    }

    async build(rootDir, sessionId, outputDir, tilesets, fullTilesets, mapName, mapTitle, globalTileOptions)
    {
        let generateAnnotated = true === sc.get(globalTileOptions, 'generateAnnotatedImages', false);
        TilesetImagePersister.persistImages(rootDir, outputDir, sessionId, fullTilesets, '', '', '', rootDir);
        ...
        this.removeStaleCroppedElements(outputDir, genTimestamp);
        ...
        let annotatedDir = FileHandler.joinPaths(generatedFolder, TilesetConst.GENERATED_ANNOTATED_SUB_FOLDER);

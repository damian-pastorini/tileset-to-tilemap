...
class TilesetFilesBuilder
{
    ...
    async cropElement(imageBuffer, element, tileset, croppedDir, tilesetBasename)
    {
        ...
        let clusterCropper = new ClusterCropper();
    ...
    async buildTilesetFilesEntries(sessionId, outputDir, tilesets, genTimestamp, croppedElementsPaths)
    {
        ...
        for(let tileset of tilesets){
            ...
            let outputFilename = this.resolveOutputFilename(desiredFilename, usedFilenames);
    ...
    buildCompositeEntries(sessionId, outputDir, tilesetsBySize, mapName, mapTitle)
    {
        ...
        for(let sizeKey of sizeKeys){
            ...
            let sizeMapName = sc.get(sizeTilesets[0], 'mapName', mapName);
            let sizeMapTitle = sc.get(sizeTilesets[0], 'mapTitle', mapTitle);
            FileHandler.writeFile(
                FileHandler.joinPaths(outputDir, configFilename),
                sc.toJsonString({
                    compositeElementsFile: compositeFilename,
                    mapsInformation: [{mapName: sizeMapName, mapTitle: sizeMapTitle}]
                }, null, 4)
            );
    ...
    async build(sessionId, outputDir, tilesets, fullTilesets, mapName, mapTitle)
    {
        TilesetImagePersister.persistImages(outputDir, sessionId, fullTilesets, '', '', '');

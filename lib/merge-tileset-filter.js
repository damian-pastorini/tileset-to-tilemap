...
const { CLUSTER_TYPE } = require('./utils/constants');
...
class MergeTilesetFilter
{
    filterTilesetElements(tileset, includeElements, includeClusters)
    {
        ...
        for(let element of tileset.elements){
            ...
            if(includeElements && CLUSTER_TYPE !== element.type){
                filtered.push(element);
                continue;
            }
            if(includeClusters && CLUSTER_TYPE === element.type){
                filtered.push(element);
            }
        }
        return filtered;
    ...
    buildElementJsonIfNeeded(element, tileset, outputDir, tilesetBasename)
    {
        ...
        let elementPath = FileHandler.joinPaths(outputDir, elementFilename);

...
class ClusterNamer
{
    ...
    parseMultiElementResponse(rawText)
    {
        ...
        return result.filter(parsedElement => parsedElement.layers.length);
    }
    ...
    {
        let lines = rawText.split('\n').map(l => l.trim()).filter(l => l.length && !l.startsWith('```'));
        let parsedLayers = this.deduplicateLayers(this.parseLayerLines(lines), relativeTiles);
    ...
    async nameElement(provider, buffer, relativeTiles, cropRows, cropCols, onToken)
    {
        ...
        return {
            name,
            layers: await this.assignLayers(provider, buffer, relativeTiles, cropRows, cropCols, onToken)
                || [{type: 'collisions', tiles: relativeTiles}]
        };
    }
    ...
    async verifyElement(provider, buffer, result, relativeTiles, cropRows, cropCols, onToken)
    {
        ...
        let parsedLayers = this.parseLayerAssign(rawText, relativeTiles);
        if(!parsedLayers){
            return result;
        }
        return {name: result.name, layers: parsedLayers};
    }

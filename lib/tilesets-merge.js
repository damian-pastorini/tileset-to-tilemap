...
class TilesetsMerge
{
    ...
    async buildTilesetOutput(tileset, outputDir)
    {
        ...
        for(let element of tileset.elements){
            ...
            FileHandler.writeFile(
                FileHandler.joinPaths(outputDir, elementFilename),
    ...
    remapPlacements(placements)
    {
        ...
        return mergedElements.map((el, i) => Object.assign({}, el, {colorIndex: i}));
    }

    buildMergedTilesetState(
        mergedName,
        mergedImagePath,
        imageWidth,
        imageHeight,
        ...
        return {
            imageId: mergedName+'.png',
            imageUrl: '/download/'+sessionId+'/'+mergedName+'.png',
            filename: mergedName+'.png',
            filePath: mergedImagePath,
            imageWidth,
            imageHeight,
            tileWidth: refTileWidth,
            tileHeight: refTileHeight,
            spacing: 0,
            margin: 0,
            tilesetColumns: mergedColumns,
            tileRows: mergedRows,
            tileCount: mergedColumns * mergedRows,
            filteredTiles: [],
            bgColor: null,
            sessionId,
    ...
    async run(sessionId, tilesetsMergeData, outputDir, onProgress)
    {
        ...
        if(2 > filteredTilesets.length){
            return {error: 'At least 2 tilesets with elements required for merge'};
        }

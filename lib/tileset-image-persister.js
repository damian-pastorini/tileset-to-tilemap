...
class TilesetImagePersister
{
    ...
    static ensureTilesetCopied(tileset, inputDir, sessionId)
    {
        ...
        if(!FileHandler.exists(tileset.filePath)){
            Logger.error('TilesetImagePersister.ensureTilesetCopied: source file not found: '+tileset.filePath);
            return false;
        }
    ...
    static ensureOutputImages(rootFolder, sessionId, tilesetSessionsDir)
    {
        ...
        for(let tileset of state.tilesets){
            ...
            let outputPath = FileHandler.joinPaths(rootFolder, tileset.imageId);

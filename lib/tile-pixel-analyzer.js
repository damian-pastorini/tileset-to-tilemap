...
class TilePixelAnalyzer
{
    ...
    computeTileVariance(data, imageWidth, channels, tileX, tileY, tileWidth, tileHeight)
    {
        ...
        for(let py = tileY; py < tileY + tileHeight; py++){
            for(let px = tileX; px < tileX + tileWidth; px++){
                let pixelIndex = (py * imageWidth + px) * channels;
                if(4 === channels && data[pixelIndex + 3] < 10){
                    continue;
                }
        ...
        return (sumSq / count) - ((sum / count) * (sum / count));
    }

    isBackgroundPixel(data, pixelIndex, bgColor, colorThreshold)
    {
        if(!bgColor){
            return false;
        }
    ...
    isTileEmpty(
        ...
        for(let py = tileY; py < tileY + tileHeight; py++){
            for(let px = tileX; px < tileX + tileWidth; px++){
                let pixelIndex = (py * imageWidth + px) * channels;
                if(4 === channels && data[pixelIndex + 3] <= alphaThreshold){
                    continue;
                }
                if(this.isBackgroundPixel(data, pixelIndex, bgColor, colorThreshold)){
                    continue;
                }
                nonEmptyCount++;
            }
    ...
    buildMaskData(
        ...
        for(let y = 0; y < imageHeight; y++){
            for(let x = 0; x < imageWidth; x++){
                let pixelIndex = (y * imageWidth + x) * channels;
